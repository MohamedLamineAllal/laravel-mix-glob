...
const expectedOutMapping = ({ defaultOut }) =>
  glb.replaceExtension(defaultOut, '.js');

const withSpecifierExpectedOutMapping = ({ defaultOut }) =>
  glb.replaceExtension(glb.removeSpecifier(defaultOut, 'c'), '.js');
  ...
  describe(`${pathType} path`, () => {
    describe('with specifier', () => {
      ...
      test('that all specifier files are compiled and mapped correctly', async () => {
        const srcSpecifierFiles = await fastGlob(`${srcPath}/**/*.c.ts`);
        ...
        try {
          const filesObjs = await Promise.all(
            checkOutFiles({
              ...
              expectedOutMapping: withSpecifierExpectedOutMapping,
            }),
          );
    ...
    describe('without specifier', () => {
      ...
      test('that specifier els are left', async () => {
        const withSpecifierFiles = await fastGlob(`${srcPath}/**/*.c.ts`);
      ...
      test('that all files are compiled and mapped correctly', async () => {
        const srcFiles = await fastGlob(`${srcPath}/**/*.ts`);
